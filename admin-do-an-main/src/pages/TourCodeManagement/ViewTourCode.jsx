...
import UpdateTourCode from './UpdateTourCode';

const { Text } = Input;
const { Title } = Typography;
function ViewTourCode() {
    const { id } = useParams()
    ...
    useEffect(() => {
        const getProduct = async () => {
            const response = await fetch(`http://127.0.0.1:8000/categoryitem/${id}`);
